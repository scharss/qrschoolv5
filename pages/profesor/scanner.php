...
<body>
    ...
    <script>
        function onScanSuccess(decodedText, decodedResult) {
            ...
            // El decodedText puede ser un JSON con ID y hash o solo un ID
            try {
                // Intentar parsear el JSON
                let qrData;
                try {
                    // Verificar que sea formato JSON con hash
                    const jsonData = JSON.parse(decodedText);
                    if (jsonData && jsonData.id && jsonData.hash) {
                        qrData = decodedText; // Mantener el formato JSON como string
                    } else {
                        throw new Error('Formato JSON incompleto');
                    }
                } catch (jsonError) {
                    // Si no es JSON válido o no tiene hash, mostrar error
                    throw new Error('Formato de QR inválido. Se requiere formato con hash.');
                }
            ...
            .fail(function() {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Formato de QR inválido. Se requiere formato con hash.',
                    timer: 3000,
                    showConfirmButton: false
                }).then(() => {
                    html5QrcodeScanner.resume();
