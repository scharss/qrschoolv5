...
try {
    ...
    try {
        ...
        // Generar un identificador único cifrado con SHA-256 usando nombre y documento
        $dataToHash = $nombre . $apellidos . $documento . $estudiante_id;
        $hashedId = hash('sha256', $dataToHash);
        
        // Crear un formato que incluya el ID real para poder decodificarlo después
        $qrData = json_encode([
            'id' => $estudiante_id,
            'hash' => $hashedId
        ]);
        
        // Generar el código QR con los datos cifrados
